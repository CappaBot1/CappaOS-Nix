...

{
  ...
  environment.etc."cappaos-defaults/copyq.conf".source =
    ./default-configs/copyq.conf;

  systemd.user.services.cappaos-copyq-init = {
    description = "Install CappaOS CopyQ configuration";
    ...
    script = ''
      echo "[CappaOS] Installing CopyQ configuration"

      install -d "$HOME/.config/copyq"

      if [ ! -f "$HOME/.config/copyq/copyq.conf" ]; then
        touch "$HOME/.config/copyq/cappaos-managed"
      fi

      if [ -f "$HOME/.config/copyq/cappaos-managed" ]; then
        cp -f \
          /etc/cappaos-defaults/copyq.conf \
